...
RUN cd /tmp && \
        ...
        tar zxvf spark-2.4.6-bin-hadoop2.7.tgz -C /usr/local --owner root --group root --no-same-owner && \
        rm spark-2.4.6-bin-hadoop2.7.tgz
# Rename Spark folder
RUN cd /usr/local && ln -s spark-2.4.6-bin-hadoop2.7 spark
...
RUN wget http://mirrors.advancedhosters.com/apache/incubator/livy/0.7.0-incubating/apache-livy-0.7.0-incubating-bin.zip
